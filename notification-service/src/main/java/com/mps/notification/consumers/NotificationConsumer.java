package com.mps.notification.consumers;

import com.mps.notification.dtos.NotificationCommandDto;
import com.mps.notification.enums.NotificationStatus;
import com.mps.notification.models.NotificationModel;
import com.mps.notification.services.NotificationService;
...
public class NotificationConsumer {
    ...
    @RabbitListener(bindings = @QueueBinding(
            value = @Queue(value = "${mps.broker.queue.notificationCommandQueue.name}", durable = "true"),
            exchange = @Exchange(value = "${mps.broker.exchange.notificationCommandExchange}", type = ExchangeTypes.TOPIC, ignoreDeclarationExceptions = "true"),
            key = "${mps.broker.key.notificationCommandKey}")
    )
